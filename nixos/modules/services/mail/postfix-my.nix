...

{
  ...
  config = mkIf config.services.postfixmy.enable {
    ...
    environment = {
      etc.postfix = { source = "/var/postfix/conf"; };
    ...
    users.groups."${setgidGroup}" = {
          gid = config.ids.gids.postdrop;
    };

    systemd.services.postfixmy =
      let
        ...
        '';
      in {
        description = "my Postfix mail server";
