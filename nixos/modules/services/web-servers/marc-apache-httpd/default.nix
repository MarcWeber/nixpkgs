...
let
  ...
  mainCfg = config.services.marc_httpd;
...

{
  ...
  config = mkIf config.services.marc_httpd.enable {
    ...
    users.users = mkIf (mainCfg.user == "wwwrun")
      { "wwwrun" = {
        group = mainCfg.group;
    ...
    users.groups = mkIf (mainCfg.group == "wwwrun") 
      { "wwwrun"  ={
    ...
    services.marc_httpd.phpOptions =
      ''
    ...
    services.marc_httpd.extraModules = mkBefore [
