    ...
    composableDerivation.composableDerivation {
      ...
      flags = {
        ...
        systemd_socket_activation = {
          patches = [ (if php734 then ./systemd-socket-activation-7.4.patch else  ./systemd-socket-activation.patch)  ];
          buildInputs = [ systemd ];
          preConfigure = ''
      ...
      preBuild = ''
        sed -i 's@#define PHP_PROG_SENDMAIL	.*@#define PHP_PROG_SENDMAIL	"${fixed.sendmail or "/run/wrappers/bin/sendmail"}"@' main/build-defs.h
      '';
